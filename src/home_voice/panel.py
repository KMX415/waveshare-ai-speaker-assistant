    ...
    def status(self):
        if self.board.info.get("standalone"):
            ...
            return dict(device=self.board.info,levels=self.board.levels,active=self.busy(),
                        ...
                        wake=getattr(self.board,"wake",{}),failure=getattr(self.board,"failure",{}),playback=getattr(self.board,"playback",{}),upload=getattr(self.board,"upload",{}),microphone=getattr(self.board,"microphone",{}),crash=list(getattr(getattr(self.board,"crash",None),"lines",[])),mock=False,message=self.board.error or native.get("message","Waiting for device"),volume=self.board.volume,
